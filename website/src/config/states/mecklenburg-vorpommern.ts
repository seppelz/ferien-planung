...
export const mecklenburgVorpommern: StateInfo = {
  ...
  holidays: [
    ...
    ...(holidays.publicHolidays["2027"]["MV"] || []).map(holiday => ({
      name: holiday.name,
      type: "public" as const,
      isRegional: true,
      start: holiday.start,
  ...
  schoolHolidays: holidays.schoolHolidays["2027"]["MV"].map(holiday => {
    ...
    const baseHolidayName = holiday.name
      ...
      .filter(part => !part.includes("mecklenburg") && !part.includes("vorpommern") && !part.includes("2025") && !part.includes("("))  // Remove state, year, and extra info
