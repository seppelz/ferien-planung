...
export const sachsenAnhalt: StateInfo = {
  ...
  holidays: [
    ...
    ...(holidays.publicHolidays["2027"]["ST"] || []).map(holiday => ({
      name: holiday.name,
      type: "public" as const,
      isRegional: true,
      start: holiday.start,
  ...
  schoolHolidays: holidays.schoolHolidays["2027"]["ST"].map(holiday => {
    ...
    const baseHolidayName = holiday.name
      ...
      .filter(part => !part.includes("sachsen") && !part.includes("anhalt") && !part.includes("2025") && !part.includes("("))  // Remove state, year, and extra info
