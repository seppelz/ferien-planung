...
export const berlin: StateInfo = {
  ...
  holidays: [
    ...
    ...(holidays.publicHolidays["2027"]["BE"] || []).map(holiday => ({
      name: holiday.name,
      type: "public" as const,
  ...
  schoolHolidays: holidays.schoolHolidays["2027"]["BE"].map(holiday => {
    ...
    const baseHolidayName = holiday.name
      ...
      .filter(part => !part.includes("berlin") && !part.includes("2025") && !part.includes("("))  // Remove state, year, and extra info
