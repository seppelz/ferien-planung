...
export const saarland: StateInfo = {
  ...
  holidays: [
    ...
    ...(holidays.publicHolidays["2027"]["SL"] || []).map(holiday => ({
      name: holiday.name,
      type: "public" as const,
      isRegional: true,
      start: holiday.start,
  ...
  schoolHolidays: holidays.schoolHolidays["2027"]["SL"].map(holiday => {
    ...
    const baseHolidayName = holiday.name
      ...
      .filter(part => !part.includes("saarland") && !part.includes("2025") && !part.includes("("))  // Remove state, year, and extra info
